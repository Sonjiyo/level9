...
// 2. 상속(inheritance) : 자식클래스가 부모클래스를 가지고 있다, 한 부모만 상속받을 수 있다.
// 3. 추상화(abstract) : 미완성된 메서드가 단 한개라도 존재하면 추상클래스, 
					// 추상클래스 객체생성 불가능 -> 상속통해 자식클래스 구현 완성
// 4. 다형성(polymorphism) : 다양한 + 형태 => 여러 자식클래스를 한개의 부모타입으로 묶을 수 있다.
						// 자식과 부모의 형변환이 가능하다.
...
// 상속은 자식클래스 extends 부모클래스 이름

// 자식 클래스는 부모가 생성 된 후에 생성됨
class C1 extends A1{
	int e;
	int f;
	C1(){
		...
		System.out.println(super.a);//부모의 위치로 가서 a에 접근
		//super(); 부모의 생성자는 자식 생성자의 최상위에 위치
	}
}
// 상속받은 자식 클래스
